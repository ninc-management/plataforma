...
export class ExpansiveGridComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (!this.maxRows) this.maxRows = this.minRows * 2;
    ...
    this.minHeight = cardHeaderHeight + cardBodyPadding + gridMargin + rowHeight * this.minRows;
    this.maxHeight = cardHeaderHeight + cardBodyPadding + gridMargin + rowHeight * this.maxRows;
  }

  toggleExpansive(): void {
    if (this.items.length > this.maxRows && this.isExpanded) this.cardBody.nativeElement.scrollTop = 0;
