...
export class Grid {
  ...
  source: DataSource = new LocalDataSource();
  settings: any;
  ...
  constructor(source: DataSource = new LocalDataSource(), settings: object = {}) {
  ...
  setSource(source: DataSource) {
    ...
    this.source
      ...
      .subscribe((data: any) => {
        ...
        if (changedRow) changedRow.setData(data);
      });
  }
  ...
  create(row: Row, confirmEmitter: EventEmitter<any>) {
    ...
    deferred.promise
      ...
      .catch((err) => {
        console.error('Smart-Table create():', err);
      });
  ...
  save(row: Row, confirmEmitter: EventEmitter<any>) {
    ...
    deferred.promise
      ...
      .catch((err) => {
        console.error('Smart-Table save():', err);
      });
  ...
  delete(row: Row, confirmEmitter: EventEmitter<any>) {
    ...
    deferred.promise
      ...
      .catch((err) => {
        console.error('Smart-Table delete():', err);
      });
  ...
  private getRowIndexToSelect(): number {
     ...
     */
    return switchPageToSelectedRowPage && selectedRowIndex < dataAmount && selectedRowIndex >= 0
